...
public class LogSplitterSequenceFilteringCancelAware implements LogSplitter {
    ...
    public static LogSplitResult split(IMLog log, Collection<Set<XEventClass>> partition, MinerState minerState) {
        // MODIFIED VERSION -- variant of LogSplitterSequenceFiltering
        // ADDED -- remove post-error-trigger empty traces
            ...
            for (IMTrace trace : log) {
                    ...
                    for (Iterator<Set<XEventClass>> itSigma = partition.iterator(); itSigma.hasNext();) {
                            ...
                            if (endedWithErrorTrigger && subtrace.isEmpty()) {
                                mapSigma2TraceIterator.get(sigma).remove();
                            }
                    }
            }
